...
!====================================================================================================
   ...
   character(len=16) :: microp_scheme   ! Microphysics scheme name
   integer :: errcode                   ! Error code for pbuf_get_index
   ...
   ! P3 tendency field indices were set during output_aerocom_aie_register() 
   ! via pbuf_add_field() calls - just log them here
   autocl_idx = pbuf_get_index('IE_AUTOCONV')
   accretl_idx = pbuf_get_index('IE_ACCRET')

   if (masterproc) write(iulog,*) 'output_aerocom_aie_init: P3_qc2qr_autoconv_tend idx=', autocl_idx
   if (masterproc) write(iulog,*) 'output_aerocom_aie_init: P3_qc2qr_accret_tend idx=', accretl_idx
   
   if (autocl_idx < 0) then
      if (masterproc) write(iulog,*) 'WARNING: P3_qc2qr_autoconv_tend index is invalid'
   end if
   
   if (accretl_idx < 0) then
      if (masterproc) write(iulog,*) 'WARNING: P3_qc2qr_accret_tend index is invalid'
   end if
