...
files300 = sorted(glob.glob(os.path.join(BASE_DIR, "*_RCE_large300_cfv0-profiles.nc")))

pairs = []
for f300 in files300:
    stem300 = os.path.basename(f300)
    model = stem300.replace("_RCE_large300_cfv0-profiles.nc", "")
    f305 = os.path.join(BASE_DIR, f"{model}_RCE_large305_cfv0-profiles.nc")
...
# target isotherms
T_targets = [220.0, 230.0, 240.0]

fig, axs = plt.subplots(1, 3, figsize=(15, 5), constrained_layout=True)

for ax, T_target in zip(axs, T_targets):
    ...
    for model, d300, d305 in profiles:
        ...
        idx = np.argmin(np.abs(T_l - T_target))

        cf0 = d300["cf_l"][idx]
        cf1 = d305["cf_l"][idx]

        tau0 = d300["tau_i_sub_l"][idx]
        tau1 = d305["tau_i_sub_l"][idx]

        if not (np.isfinite(cf0) and np.isfinite(cf1) and np.isfinite(tau0) and np.isfinite(tau1)):
            continue

        dln_CF_vals.append((cf1 - cf0) / (0.5 * (cf1 + cf0)))
        dln_tau_vals.append((tau1 - tau0) / (0.5 * (tau1 + tau0)))

    x = np.array(dln_tau_vals)
    y = np.array(dln_CF_vals)
    ...
    # plot
    ax.scatter(x, y, s=70, edgecolor='k')
    ...
    ax.set_title(f"T = {int(T_target)} K\nSlope={slope:.2f}, R²={r2:.2f}")
    ax.set_xlabel(r"$\Delta \ln \tau_{i,\rm sub}$")
    ax.set_ylabel(r"$\Delta \ln \mathrm{CF}$")
